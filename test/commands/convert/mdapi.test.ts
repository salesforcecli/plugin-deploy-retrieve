...
import { Mdapi } from '../../../src/commands/project/convert/mdapi';
...
describe('project convert mdapi', () => {
  ...
  class TestConvert extends Mdapi {
    public async runIt() {
      await this.init();
      return this.run();
    }
  ...
  it('should error when rootdir is not a directory', async () => {
    ...
    try {
      await runConvertCmd(['--root-dir', fileName]);
      assert(false, `expected ${commandName} to error`);
    } catch (e: unknown) {
      expect((e as Error).name).to.equal('InvalidFlagPathError');
  ...
  it('should error when outputdir is not a directory', async () => {
    ...
    try {
      await runConvertCmd(['--root-dir', rootdir, '--output-dir', fileName]);
      assert(false, `expected ${commandName} to error`);
    } catch (e: unknown) {
      expect((e as Error).name).to.equal('InvalidFlagPathError');
      expect((e as Error).message).to.include(trimmedFileName);
  ...
  it('should error when metadatapath does not exist', async () => {
    ...
    try {
      await runConvertCmd(['--root-dir', rootdir, '--metadata-path', fileName]);
      assert(false, `expected ${commandName} to error`);
    } catch (e: unknown) {
  ...
  it('should error when manifest does not exist', async () => {
    ...
    fsStatStub.withArgs(resolvedFilePath).throws(err);
    try {
      await runConvertCmd(['--root-dir', rootdir, '--manifest', fileName]);
      assert(false, `expected ${commandName} to error`);
    } catch (e: unknown) {
      expect((e as Error).name).to.equal('InvalidFlagPath');
      expect((e as Error).message).to.include(trimmedFileName);
  ...
  it('should create the outputdir when it does not exist', async () => {
    ...
    const result = await runConvertCmd(['--root-dir', 'rootdir', '--output-dir', resolvedOutputDir]);
  ...
  it('should return an empty array when no metadata converted (json)', async () => {
    ...
    const result = await runConvertCmd(['--root-dir', 'rootdir', '--json']);
    expect(result).to.deep.equal([]);
  ...
  it('should report when no metadata converted (display)', async () => {
    ...
    fsStatStub.returns({ isDirectory: () => true });
    const result = await runConvertCmd(['--root-dir', 'rootdir']);
    expect(result).to.deep.equal([]);
    expect(convertStub.called).to.be.false;
  });
